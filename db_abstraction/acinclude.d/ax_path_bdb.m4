...
# COPYLEFT
#
#   Copyright (c) 2005 Tim Toolan <toolan@ele.uri.edu>
#
...
AC_DEFUN([AX_PATH_BDB], [
  ...
  if test "x$with_bdb_dir" = "x" ; then
    ...
    AX_PATH_BDB_NO_OPTIONS([$1], [HIGHEST], [
     ...
     # Check for specific header file db_cxx.h
     AC_MSG_CHECKING([db_cxx.h presence in $ax_path_bdb_INC])
     if test -f "$ax_path_bdb_INC/db_cxx.h" ; then
       AC_MSG_RESULT([yes])
       # Check for library
...
AC_DEFUN([AX_PATH_BDB_NO_OPTIONS], [
  ...
  if test "x$cross_compiling" = "xyes" ; then
    ...
    # The AC_PREPROC_IFELSE confirms that db_cxx.h is preprocessable,
    # and extracts the version number from it.
    AC_MSG_CHECKING([for db_cxx.h])
...
AC_DEFUN([AX_PATH_BDB_PATH_GET_VERSION], [
  ...
  AC_MSG_CHECKING([in $1/include for db_cxx.h])
  ax_path_bdb_path_get_version_got_header=no
  test -f "$1/include/db_cxx.h" && ax_path_bdb_path_get_version_got_header=yes
  AC_MSG_RESULT([$ax_path_bdb_path_get_version_got_header])

  # Indicate status of checking for Berkeley DB library.
  AC_MSG_CHECKING([in $1/lib for library -ldb_cxx])
...
int main(int argc,char **argv)
{
  ...
  if test "x$HEADER_VERSION" != "x" ; then
    ...
    if test "$ax_path_bdb_env_confirm_lib_ok" = "no" ; then
      # try format "db"
      TEST_LIBNAME='-ldb_cxx'
      AX_PATH_BDB_ENV_CONFIRM_LIB([$HEADER_VERSION], [$TEST_LIBNAME])
    fi

    if test "$ax_path_bdb_env_confirm_lib_ok" = "no" ; then
      # try format "db-X.Y"
      TEST_LIBNAME="-ldb_cxx-${MAJOR}.$MINOR"
      AX_PATH_BDB_ENV_CONFIRM_LIB([$HEADER_VERSION], [$TEST_LIBNAME])
    fi
